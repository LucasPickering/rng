...
# COPIED FROM https://github.com/Zack--/phonetics WITH MINIMAL ALTERATIONS
...
def process_C(s, idx, last, slavogermanic):
    if (
        idx > 1
        and not utils.isvowel(s.get(idx - 2))
        and s.get(idx - 1, 3) == "ACH"
        and (
            s.get(idx + 2) != "I"
            and (
                s.get(idx + 2) != "E"
                or s.get(idx - 2, 6) in {"BACHER", "MACHER"}
            )
        )
    ):
        return ("K", "K", 2)
        ...
        if idx > 0 and s.get(idx, 4) == "CHAE":
            ...
            idx == 0
            and (
                s.get(idx + 1, 5) in {"HARAC", "HARIS"}
                or s.get(idx + 1, 3) in {"HOR", "HYM", "HIA", "HEM"}
            )
            and s.get(idx, 5) != "CHORE"
        ):
            return ("K", "K", 2)
        elif (
            (s.get(0, 4) in {"VAN ", "VON "} or s.get(0, 3) == "SCH")
            or s.get(idx - 2, 6) in {"ORCHES", "ARCHIT", "ORCHID"}
            or s.get(idx + 2) in {"T", "S"}
            or (
                (s.get(idx - 1) in {"A", "O", "U", "E"} or idx == 0)
                and s.get(idx + 2)
        ...
        if s.get(idx + 2) in {"I", "E", "H"} and not s.get(idx + 2, 2) == "HU":
            if (idx == 1 and s.get(idx - 1) == "A") or s.get(idx - 1, 5) in {
                "UCCEE",
                "UCCES",
            }:
                return ("KS", "KS", 3)
        ...
        if s.get(idx + 1, 2) in {" C", " K", " Q", " G"}:
            return ("K", "K", 3)
        elif s.get(idx + 1) in {"C", "K", "Q"} and s.get(idx + 1, 2) not in {
            "CE",
            "CI",
        }:
            return ("K", "K", 2)
...
def process_G(s, idx, last, slavogermanic):
    if s.get(idx + 1) == "H":
        if idx > 0 and not utils.isvowel(s[idx - 1]):
            ...
            (idx > 1 and s.get(idx - 2) in {"B", "H", "D"})
            or (idx > 2 and s.get(idx - 3) in {"B", "H", "D"})
            or (idx > 3 and s.get(idx - 4) in {"B", "H"})
        ):
            return ("", "", 2)
        else:
            if (
        ...
        (s.get(idx + 1, 2) == "ER" or s.get(idx + 1) == "Y")
        and s.get(0, 6) not in {"DANGER", "RANGER", "MANGER"}
        and s.get(idx - 1) not in {"E", "I"}
        and s.get(idx - 1, 3) not in {"RGY", "OGY"}
    ):
        return ("K", "J", 2)
    elif s.get(idx + 1) in {"E", "I", "Y"} or s.get(idx - 1, 4) in {
        "AGGI",
        "OGGI",
    }:
        if (s.get(0, 4) in {"VAN ", "VON "} or s.get(0, 3) == "SCH") or s.get(
            idx + 1, 2
        ) == "ET":
            return ("K", "K", 2)
        else:
...
def process_H(s, idx, last, slavogermanic):
    if (idx == 0 or utils.isvowel(s.get(idx - 1))) and utils.isvowel(
        s.get(idx + 1)
    ):
        return ("H", "H", 2)
    else:
    ...
    if s.get(idx, 4) == "JOSE" or s.get(0, 4) == "SAN ":
        ...
        if idx == 0 and not s.get(idx, 4) == "JOSE":
            ...
            if (
                ...
                and not slavogermanic
                and (s.get(idx + 1) == "A" or s.get(idx + 1) == "O")
            ):
                primary = "J"
                secondary = "H"
            else:
...
def process_L(s, idx, last, slavogermanic):
    ...
    if s.get(idx + 1) == "L":
        if not (
            (
                idx == len(s) - 3
                and s.get(idx - 1, 4) in {"ILLO", "ILLA", "ALLE"}
            )
            or (
                (
                    s.get(len(s) - 2, 2) in {"AS", "OS"}
                    or s.get(last) in {"A", "O"}
                )
                and s.get(idx - 1, 4) == "ALLE"
            )
        ):
            secondary = "L"
        return (primary, secondary, 2)
...
def process_M(s, idx, last, slavogermanic):
    if (
        s.get(idx - 1, 3) == "UMB"
        and (idx + 1 == last or s.get(idx + 2, 2) == "ER")
    ) or s.get(idx + 1) == "M":
        return ("M", "M", 2)
    return ("M", "M", 1)
...
def process_R(s, idx, last, slavogermanic):
    ...
    if not (
        idx == last
        and not slavogermanic
        and s.get(idx - 2, 2) == "IE"
        and not s.get(idx - 4, 2) in {"ME", "MA"}
    ):
        primary = "R"
...
def process_S(s, idx, last, slavogermanic):
    if s.get(idx - 1, 3) in {"ISL", "YSL"}:
        ...
        if not slavogermanic:
            ...
            return ("S", "S", 3)
    elif (idx == 0 and s.get(idx + 1) in {"M", "N", "L", "W"}) or s.get(
        idx + 1
    ) == "Z":
...
def process_T(s, idx, last, slavogermanic):
    if s.get(idx, 4) == "TION" or s.get(idx, 3) in {"TIA", "TCH"}:
        ...
        if (
            s.get(idx + 2, 2) in {"OM", "AM"}
            or s.get(0, 4) in {"VAN ", "VON "}
            or s.get(0, 3) == "SCH"
        ):
            return ("T", "T", 2)
        else:
...
def process_W(s, idx, last, slavogermanic):
    if s.get(idx, 2) == "WR":
        ...
        (idx == last and utils.isvowel(s.get(idx - 1)))
        or s.get(idx - 1, 5) in {"EWSKI", "EWSKY", "OWSKI", "OWSKY"}
        or s.get(0, 3) == "SCH"
    ):
        return ("", "F", 1)
    elif s.get(idx, 4) in {"WICZ", "WITZ"}:
        return ("TS", "FX", 4)
    elif idx == 0 and (utils.isvowel(s.get(idx + 1)) or s.get(idx, 2) == "WH"):
...
def process_X(s, idx, last, slavogermanic):
    ...
    if not (
        idx == last
        and (
            s.get(idx - 3, 3) in {"IAU", "EAU"}
            or s.get(idx - 2, 2) in {"AU", "OU"}
        )
    ):
        primary = "KS"
        secondary = "KS"
...
def dmetaphone_segments(source):
    """
    This has been modified from the original source to return segmented
    metaphones, instead of appending them all into one string. It also returns
    the corresponding segments from the original source.
    """

    source = utils.LazyString(source.upper())
    slavogermanic = utils.isslavogermanic(source)
    last = len(source) - 1
    segments, index = check_start(source)
    ...
    while index < len(source):
        ...
        if func:
            ...
            if result:
                primary, secondary, chars_consumed = result
                if primary == secondary:
                    secondary = None

                next_index = index + chars_consumed
                segments.append(
                    (source[index:next_index], (primary, secondary))
                )
                index = next_index
            else:
                index += 1
